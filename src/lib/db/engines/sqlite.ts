...
export class SQLite implements DBStrategy {
	...
	async init(): Promise<void> {
		try {
			...
			const vfs = this.#dbOptions.persistent
				? await IDBBatchAtomicVFS.create(this.#dbName, module, { lockPolicy: 'shared+hint' })
				: await MemoryAsyncVFS.create(this.#dbName, module);
