...
export default class IDBBatchAtomicVFS extends WebLocksMixin(FacadeVFS) {
	...
	async jOpen(zName, fileId, flags, pOutFlags) {
		try {
			...
			if (!meta && flags & CONSTS.SQLITE_OPEN_CREATE) {
				meta = {
					name: path,
					fileSize: 0,
					version: 0
	...
	async jDelete(zName, syncDir) {
		try {
			...
			this.lastError = e;
			return CONSTS.SQLITE_IOERR_DELETE;
		}
	}
	...
	async jAccess(zName, flags, pResOut) {
		try {
			...
			this.lastError = e;
			return CONSTS.SQLITE_IOERR_ACCESS;
		}
	}
	...
	async jClose(fileId) {
		try {
			...
			this.lastError = e;
			return CONSTS.SQLITE_IOERR_CLOSE;
		}
	}
	...
	async jRead(fileId, pData, iOffset) {
		try {
			...
			this.lastError = e;
			return CONSTS.SQLITE_IOERR_READ;
		}
	}
	...
	jWrite(fileId, pData, iOffset) {
		try {
			const file = this.mapIdToFile.get(fileId);
			if (file.flags & CONSTS.SQLITE_OPEN_MAIN_DB) {
				if (!file.rollback) {
			...
			if (
				!isOverwrite ||
				file.flags & CONSTS.SQLITE_OPEN_MAIN_DB ||
				file.flags & CONSTS.SQLITE_OPEN_TEMP_DB
			) {
				const block = {
					path: file.path,
					offset: -iOffset,
	...
	jTruncate(fileId, iSize) {
		try {
			...
			this.lastError = e;
			return CONSTS.SQLITE_IOERR_TRUNCATE;
		}
	}
	...
	async jSync(fileId, flags) {
		try {
			...
			this.lastError = e;
			return CONSTS.SQLITE_IOERR_FSYNC;
		}
	}
	...
	jFileSize(fileId, pSize64) {
		try {
			...
			this.lastError = e;
			return CONSTS.SQLITE_IOERR_FSTAT;
		}
	}
	...
	async jLock(fileId, lockType) {
		...
		if (lockType === CONSTS.SQLITE_LOCK_SHARED) {
			// Update metadata.
	...
	jFileControl(fileId, op, pArg) {
		try {
			const file = this.mapIdToFile.get(fileId);
			switch (op) {
				case CONSTS.SQLITE_FCNTL_PRAGMA: {
	...
	jDeviceCharacteristics(pFile) {
		return 0 | CONSTS.SQLITE_IOCAP_BATCH_ATOMIC | CONSTS.SQLITE_IOCAP_UNDELETABLE_WHEN_OPEN;
	}
			...
			request.onupgradeneeded = async (event) => {
				...
				switch (event.oldVersion) {
					case 0:
						...
						db.createObjectStore('blocks', { keyPath: ['path', 'offset', 'version'] }).createIndex(
							'version',
							['path', 'version']
						);
