<script lang="ts">
	...
	const closeDB = () => {
		...
		message = `Closing ${dbName} Database`;
		dbWorkerService.sendMessage({ command: 'CLOSE_DB', args: { dbName } });
		disabled = true;
	};
	...
	const unsubscribe = dbWorkerService.responses.subscribe((response) => {
		dbName = '';
		disabled = false;
		loading = response.status === 'LOADING' && response.command === 'CLOSE_DB';

		if (response.status === 'SUCCESS' && response.command === 'GET_ACTIVE_DBS') {
			loading = false;
			availableDBs = response.data.activeDBs;
		} else if (response.status === 'SUCCESS' && response.command === 'CLOSE_DB') {
			loading = false;
			message = 'Closed DB: ' + response.data.dbName;
		} else if (response.status === 'ERROR' && response.command === 'CLOSE_DB') {
...
{:else}
	<div>
		<h2 class="text-lg font-semibold">Close Database</h2>
		<form onsubmit={closeDB} class="mt-4 space-y-4">
			<div>
