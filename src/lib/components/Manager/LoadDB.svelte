<script lang="ts">
	...
	const loadDB = () => {
		...
		message = `Loading ${dbName} Database`;
		dbWorkerService.sendMessage({ command: 'LOAD_DB', args: { dbName } });
		disabled = true;
	};
	...
	const unsubscribe = dbWorkerService.responses.subscribe((response) => {
		...
		} else if (response.status === 'SUCCESS' && response.command === 'LOAD_DB') {
			loading = false;
			message = 'Loaded DB: ' + response.data.dbName;
		} else if (response.status === 'ERROR' && response.command === 'LOAD_DB') {
	...
	onMount(() => {
		dbWorkerService.sendMessage({ command: 'GET_AVAILABLE_DBS' });
	});
...
{:else}
	<div>
		<h2 class="text-lg font-semibold">Load Database</h2>
		<form onsubmit={loadDB} class="mt-4 space-y-4">
			<div>
